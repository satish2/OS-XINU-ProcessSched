...
struct	pentry	{
	...
	int	pwaitret;
	/* Parameters for LINUXSCHED/MULTIQSCHED for PA1 */
	int real; // if real is 0, then it is normal process else, it is real-time process
	int goodness; /*Goodness = Unused quantum + priority. Value Initialized in initialize.c*/
	int quantum; /*Quantum = 10 CPU ticks. Value Initialized in initialize.c */
	int quantumleft; /*As defined in question, remaining quantum CPU ticks. Value Initialized in initialize.c == Counter*/
	/* Parameters for LINUXSCHED/MULTIQSCHED for PA1 */
};
